...
if 'class AudioMeterPolicy' not in professional:
    ...
    start=professional.index('class AudioMeterPolicy')
    end=professional.index('};', start)+3
    professional=professional[:start]+addition.split('namespace olive {\n\n',1)[1]+professional[end:]
if '#include <cmath>' not in professional:
    professional='#include <cmath>\n'+professional
write('app/professional/professionalcore.h',professional)
print('Applied RB VideoFire 2.4.0 professional workspace foundation with G1 audio input safety')
